...
CREATE TABLE IF NOT EXISTS migraterl.schema_journal (
    namespace text NOT NULL CONSTRAINT namespace_len CHECK (LENGTH(namespace) <= 255),
    script_name text NOT NULL CONSTRAINT script_name_len CHECK (LENGTH(script_name) <= 255),
    content_hash text NOT NULL,
    script_class text NOT NULL DEFAULT 'once' CONSTRAINT script_class_enum CHECK (script_class IN ('once', 'on_change')),
    ...
    CONSTRAINT schema_journal_pk PRIMARY KEY (namespace, script_name, valid_period WITHOUT OVERLAPS)
);

-- Fast lookup of the currently-applied set for a namespace.
CREATE INDEX IF NOT EXISTS index_schema_journal_current ON migraterl.schema_journal (namespace, script_name)
WHERE
    UPPER(valid_period) IS NULL;
